// 若网页来源不存在，将其设置为网易体育
var source_name = "网易体育网"
// 设置爬取新闻网站的种子页面
var seedURL = 'https://sports.163.com/'
// 设置转码格式
var Encode = 'utf-8'
...
// 设置参数
rule.hour = [0,18]   // 每天凌晨0点整和下午6点整进行自动爬取数据
rule.minute = 0
rule.second = 0
            ...
            html_URL.each(function (index, body) {
                try {
                    ...
                    if (!url_reg.test(whole_URL)) return
                        ...
                        var fetch_url_Sql = 'select url from sports_info where url=?'    // 从表web_info中提取相应URL的数据
                        ...
                        mysql.query(fetch_url_Sql, fetch_url_Sql_Params, function(err, result) {
                            ...
                            } else news_get_info(whole_URL); // 未爬取过这个网页，进行新闻页面的读取
                        })
                    }
...
function news_get_info(whole_URL) {
    ...
    MyRequest(whole_URL, function (err, res, body) {
        ...
        if (!err && res.statusCode == 200) {
            ...
            var fetchAddSql = 'INSERT INTO sports_info(url,source_name,source_encoding,title,' +
            'source,author,publish_date,crawltime,content,editor) VALUES(?,?,?,?,?,?,?,?,?,?)'
            // 插入语句中相应的参数
